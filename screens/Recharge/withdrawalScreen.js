...
class Withdrawal extends React.Component {
  ...
  render() {
    ...
    if (!userSecurityLevel.isTradePassword && userSecurityLevel.withdrawTradpswd) {
      return (
    ...
    if (!userSecurityLevel.isBankUserName) {
      return (
        <View style={{backgroundColor: '#fff', paddingVertical: 25}}>
      ...
      cnyView =
        <ScrollView style={{height: height}}>
          <ImageBackground source={require('../../assets/images/withdraw_bg1.jpg')}
            ...
            <Text style={styleUtil({fontSize: 32, color: '#ffffff'})}>{Number(userBalanceInfoYE.canWithdrawBalance).toFixed(4)}</Text>
