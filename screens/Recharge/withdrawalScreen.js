...
class Withdrawal extends React.Component {
  ...
  render() {
    ...
    if (!isAllowWithdraw.sign || userConsume.code !== 0) {
      return (
        <View style={{backgroundColor: '#fff'}}>
          <Text style={stylesUtil({color: '#f15a23', fontSize: 14, lineHeight: 30, textAlign: 'center'})}>温馨提示：您当前不可提现，如有疑问请联系客服</Text>
          {
            !isAllowWithdraw.sign && <Text style={stylesUtil({color: '#f15a23', fontSize: 14, lineHeight: 30, textAlign: 'center'})}>{isAllowWithdraw.message}</Text>
          }
          <Text style={stylesUtil({color: '#f15a23', fontSize: 14, lineHeight: 30, textAlign: 'center'})}>{userConsume.message}</Text>
        </View>
      )
    }
    ...
    if (userConsume.status === 1) {
      return (
        <View style={{backgroundColor: '#fff'}}>
          <Text style={stylesUtil({color: '#f15a23', fontSize: 14, lineHeight: 30, textAlign: 'center'})}>温馨提示：{userConsume.remark}</Text>
        </View>
      )
    }
      ...
      return (
        <ScrollView style={{height: height}}>
          <ImageBackground source={require('../../assets/images/withdraw_bg1.jpg')}
            style={stylesUtil({width: '100%', height: 120, alignItems: 'center', paddingTop: 26})}>
            <Text style={stylesUtil({fontSize: 32, color: '#ffffff'})}>{userBalanceInfoYE.canWithdrawBalance}</Text>
            <Text style={stylesUtil({fontSize: 14, color: '#ffffff'})}>可提金额(元)</Text>
          </ImageBackground>
          {
            ...
            <List>
              ...
              >
                <List.Item
                  arrow="horizontal"
                  style={stylesUtil({height: 40, borderBottomWidth: 0})}
                  thumb={
            ...
            <View style={{backgroundColor: '#fff'}}>
              <Text style={stylesUtil({color: '#f15a23', fontSize: 14, lineHeight: 30, textAlign: 'center'})}>请您先前往银行卡管理页面添加银行卡！</Text>
            </View>
          }
          <View style={stylesUtil({padding: 12})}>
            <Text style={stylesUtil({color: '#a4a4a4', lineHeight: 24, fontSize: 12})}>提现限制：您今天已提现 <Text
            ...
            <Text style={stylesUtil({color: '#a4a4a4', lineHeight: 24, fontSize: 12})}>手续费说明：每日免费提现次数为 <Text
          ...
          <View style={stylesUtil({height: 200, padding: 12, alignItems: 'center'})}>
            <Text style={stylesUtil({fontSize: 12, color: '#a4a4a4'})}>提现时间：北京时间 <Text
          ...
          >
            <View style={stylesUtil({ paddingVertical: 10 })}>
              <View style={{flexDirection: 'row'}}>
                <Text style={stylesUtil({width: '10%', textAlign: 'center', color: '#555', lineHeight: 32})}>索引</Text>
                <Text style={stylesUtil({width: '35%', textAlign: 'center', color: '#555', lineHeight: 32})}>取款金额</Text>
                <Text style={stylesUtil({width: '35%', textAlign: 'center', color: '#555', lineHeight: 32})}>实际到账</Text>
                <Text style={stylesUtil({width: '20%', textAlign: 'center', color: '#555', lineHeight: 32})}>手续费</Text>
              </View>
              {
                sonOrderList.map((item, idx) => {
                  return <View style={stylesUtil({flexDirection: 'row', borderTopWidth: 0.5, borderTopColor: '#cacaca'})} key={idx}>
                    <Text style={stylesUtil({width: '10%', textAlign: 'center', color: '#555', lineHeight: 32})}>{idx+1}</Text>
                    <Text style={stylesUtil({width: '35%', textAlign: 'center', color: '#555', lineHeight: 32})}>{item.originamount}</Text>
                    <Text style={stylesUtil({width: '35%', textAlign: 'center', color: '#555', lineHeight: 32})}>{item.amount}</Text>
                    <Text style={stylesUtil({width: '20%', textAlign: 'center', color: '#555', lineHeight: 32})}>{item.fee}</Text>
                  </View>
                })
              }
              {
                sonOrderList.length === 0 &&
                <View style={stylesUtil({lineHeight: 50, borderTopWidth: 0.5, borderTopColor: '#cacaca'})}>
                  <Text style={stylesUtil({lineHeight: 50, textAlign: 'center', color: '#a4a4a4'})}>暂无数据</Text>
                </View>
              }
            </View>
          </Modal>
