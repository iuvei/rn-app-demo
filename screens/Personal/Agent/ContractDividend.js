...
class ContractDividend extends React.Component {
  ...
  editContract = ({dataItem, isConfrim}) => {
    let { id } = dataItem
    updateDividendRule({dividendType: 5, id, status: isConfrim ? 1 : 2}).then(res => {
      if (res.code === 0) {
      ...
      <View style={{flex: 1}}>
        <Tabs tabs={tabs} onChange={(tab, index) => {
          ...
          <View style={styles.container}>
            ...
            <UIListView
              ref={ref => this.downUserDividendRule = ref}
...
const mapStateToProps = (state, props) => {
  let { myDividendRule } = state.member
  return { myDividendRule }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(ContractDividend)
